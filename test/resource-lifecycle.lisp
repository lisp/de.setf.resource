...
(test:test resource.resource-lifecycle.project-graph.resource-object.0
  (let* ((p (make-instance 'person :name "name" :age 1 :parents nil)))
    (de.setf.rdf:project-graph (list (de.setf.rdf:triple (uri p) 'height 200)
                             (de.setf.rdf:triple (uri p) 'weight 70))
                       p)
...
(test:test resource.resource-lifecycle.project-graph.resource-object.1
  (let* ((p (make-instance 'person :name "name" :age 1 :parents nil))
    ...
    (de.setf.rdf:project-graph (list (de.setf.rdf:triple (uri p) 'height 200)
                             (de.setf.rdf:triple (uri p) 'weight 70))
                       p)
    (de.setf.rdf:project-graph p #'(lambda (s) (push (triple-object s) values)))
...
(test:test resource.resource-lifecycle.project-graph.resource-object.2
  (let* ((uri (uuid:make-v1-uuid))
         (class (de.setf.rdf:project-graph (list (de.setf.rdf:triple uri '{foaf}firstName "also a name")
                                         (de.setf.rdf:triple uri '{foaf}age 2)
                                         (de.setf.rdf:triple uri 'height 200)
                                         (de.setf.rdf:triple uri 'weight 70))
                                   (find-class 'person)))
         (object (de.setf.rdf:find-instance class uri))
         (expected '("also a name" 2 200 70))
         (values ()))
    (de.setf.rdf:project-graph object #'(lambda (s) (push (triple-object s) values)))
    (null (set-exclusive-or values expected :test 'equal))))
...
(test:test resource.resource-lifecycle.project-graph.resource-object.2
  (let* ((p (make-instance 'person :name "name" :age 1 :parents nil))
    ...
    (de.setf.rdf:project-graph (list (de.setf.rdf:triple (uri p) 'height 200)
                             (de.setf.rdf:triple (uri p) 'weight 70))
                       p)
    (de.setf.rdf:project-graph p source)
    (every #'(lambda (stmt) (de.setf.rdf:has-statement? source stmt))
           (list (de.setf.rdf:triple p '{foaf}firstName "name")
                 (de.setf.rdf:triple p '{foaf}age 1)
                 (de.setf.rdf:triple p 'height 200)
                 (de.setf.rdf:triple p 'weight 70)))))
...
(test:test resource.resource-lifecycle.project-graph.resource-class
 ...
 declared of type (CONS PERSON), no statement yiels the result NIL."
   (progn (de.setf.rdf:find-class 'person '{foaf}Person)
          (de.setf.rdf:find-class 'person '{foaf}Agent)
          (let ((s (class-repository '{foaf}Person)))
            (load-family s)
            (let ((people (de.setf.rdf:project-graph s (find-class '{foaf}Person))))
