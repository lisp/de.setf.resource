...
(test:test resource.resource-class.finalize-inheritance
  (progn (c2mop:finalize-inheritance (find-class 'resource-class))
         ...
         (c2mop:finalize-inheritance (find-class 'rdf-archetypal-property-definition))
         (c2mop:finalize-inheritance (find-class 'rdf-prototypal-property-definition))
...
(test:test resource.resource-class.make-instance.2
 ...
 nb. the indirect type tests have been observed to fail in mcl if it's type chace gets out of sync"
  (let ((*load-verbose* (eq test::*test-unit-mode* :verbose))
    ...
    (list (typep (class-source class) 'resource-mediator)
...
(test:test resource.resource-class.find-class
  ...
  (let* ((class (find-class 'person))
    ...
    (list (rdf:find-class class '{rdfs}Class)
         (typep (test:ignored-error (rdf:find-class (class-source class) '{foaf}Person))
                'rdf:class-not-found-error)
         (typep (rdf:find-class class '{foaf}Person)
                'resource-class)
         (class-vocabulary class)
         (typep (rdf:find-class (class-source class) '{foaf}Person)
                'resource-class))))


(test:test resource.resource-class.class-source
  ...
  (let ((person-class (find-class 'person))
    ...
    (and (typep person-class 'resource-class)
         (typep (class-source person-class) 'resource-mediator)
         ...
         (typep (class-source adult-class) 'resource-mediator)
...
;;; (rdf:query (class-source (find-class 'prototype)) :predicate '{rdf}type :object '{owl}Ontology)
;;; (unregister-value (class-source (find-class 'prototype)) '{rdf}type nil)
;;; (unregister-value (class-source (find-class 'prototype)) '{owl}Ontology nil)
;;; (rdf:repository-value (class-source (find-class 'resource-class)) '{owl}Ontology)
